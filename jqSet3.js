$(document).ready(function(){
    $(document).ready(function(){
        $products=[
            {id:101,name:"Item1",quantity:0,mrp:0,tax:0,total:0},
            {id:102,name:"Item2",quantity:0,mrp:0,tax:0,total:0},
            {id:103,name:"Item3",quantity:0,mrp:0,tax:0,total:0},
            {id:104,name:"Item4",quantity:0,mrp:0,tax:0,total:0},
            {id:105,name:"Item5",quantity:0,mrp:0,tax:0,total:0}
        ]
        $min=0;
        ...
        $("#tabItems").on("click","#btnAdd",function(){
            ...
            if($arr.length<$products.length+1){
                ...
                $($index).after("<tr><td><select class='selProducts'><option>--Select Product--</option> <option>101 Item1</option><option>102 Item2</option><option>103 Item3</option><option>104 Item4</option> <option>105 Item5</option></select></td><td><input type='text' class='inpQuantity' disabled/></td><td><input type='text' class='inpMRP' disabled /></td><td><select id='selTax' disabled> <option>0%</option><option>5%</option><option>12%</option><option>18%</option><option>28%</option></select></td><td><input type='text' class='inpTotal' disabled /></td><td><button id='btnAdd' disabled><i class='fa-solid fa-plus'></i></button><button id='btnRemove' disabled><i class='fa-solid fa-trash-can'></i></button></td></tr>")
            }
            else{
                return;
            }
        ...
        for(i=0;i<$products.length;i++){
            ...
            $products[$ind].tax=$products[$ind].total*$num/100;
            if($txt=="5%"){
